...
def main(batch_size=512, epochs = 25, period = 5):
    ...
    data = ' '.join(list(pd.read_csv(data_path, nrows = 20)['body']))
    ...
    model = Sequential()
    model.add(GRU(75, input_shape=(X_train.shape[1], X_train.shape[2])))
...
def plot_acc(history):
    ...
    plt.savefig('gru_training_acc.png')
